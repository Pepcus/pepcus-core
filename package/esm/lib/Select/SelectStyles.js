...
const getSelectStyles = ({
  ...
  const darkColor = themeGet(`palette.common.dark`)({
    theme
  });
  const inputColor = themeGet(`palette.common.input`)({
    theme
  });
  const inputDisabledColor = themeGet(`palette.common.inputDisabled`)({
    theme
  });
  const lighterColor = themeGet(`palette.common.lighter`)({
    theme
  });
  const placeholderColor = themeGet(`palette.common.placeholder`)({
    theme
  }); // Lighten / Darken the colors.

  const disabledColor = lighten(0.075, inputDisabledColor);
  const hoverColor = lighten(0.04, inputDisabledColor); // Return the select styles
  ...
  return {
    ...
    control: (provided, state) => {
      ...
      return _objectSpread({}, provided, {
        ...
        width,
        '&:hover': {
          borderColor: isFocused ? COLORS.PRIMARY : themeGet('palette.common.lighter')({
            theme
          })
        },
        transition: 'border-color 0.2s ease-in-out, box-shadow 0.2s ease-in-out',
        borderColor: themeGet('palette.common.lighter')({
          theme
        }),
    ...
    menu: (provided, state) => {
      return _objectSpread({}, provided, {
        ...
        borderStyle: 'solid',
        borderWidth: '1px',
        boxShadow: themeGet(`shadows[${elevationDirection}][${elevation}]`, boxShadow)({
          theme
        }),
        marginTop: 4,
