...
export const getZebraStyle = (index, zebra) => {
  ...
  const idx = index + 1; // Switch on the zebra prop of 'odd' or 'even'
...
export const disableTableCell = ({
  col,
  row,
  handlers
}) => {
  const disableKeys = _get(col, 'disabled'); // Return `false` if no disable keys are found.
  ...
  return row && Object.keys(disableKeys).every(key => Boolean(key in row) && Boolean(row[key] === disableKeys[key]));
};