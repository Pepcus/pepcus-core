...
export function normalizeContentfulField(data, keysList) {
  try {
    ...
    return data.map(field => {
      const updatedField = {}; // Go through all the requested keys and extract them.
...
export function normalizeInsightCategoriesList(list) {
  try {
    ...
    const categories = [...list[0].fields.categories]; // Normalize the categories list.
    ...
    const normalizedCategories = normalizeContentfulField(categories, [{
      ...
      label: 'topics',
      defaultValue: []
    }]).filter(cat => !_isEmpty(cat.topics)); // Normalize the categories topics list.

    const normalizeCategoriesWithTopics = normalizedCategories.map(cat => _objectSpread({}, cat, {
