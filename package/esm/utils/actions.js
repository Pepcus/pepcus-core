...
export const getHandler = (actions = {}, handlerRef = '', defaultValue = null) => !_isEmpty(actions) && handlerRef in actions ? actions[handlerRef] : defaultValue;
/**
...
export const replaceHandlerRefs = (items = [], handlers = {}, refProp = null, invokeWith = null) => items.map(item => {
  // Determine the `onClick` key
  const onClickKey = refProp && refProp in item ? refProp : 'onClick'; // Extract the handler via the `handlerRef` from the `item`.

  const onClick = onClickKey in item && typeof item[onClickKey] === 'function' ? item[onClickKey] : getHandler(handlers, item[onClickKey]); // Return the updated `item`.

  return _objectSpread({}, item, {
    // Update the `onClick` functionality if the `invokeWith` is available.
    onClick: !_isEmpty(invokeWith) && typeof onClick === 'function' ? () => onClick(invokeWith) : onClick
  });
});
/**
