...
export function extractErrorMessage(error) {
  ...
  const data = get(error, 'data') || get(error, 'response.data'); // Create an error message, defaulting to the error.message.

  let message = error.message || 'error occurred'; // If the data is not present, then just return the error's message;
  ...
  if (data.error_description) {
    ...
    const name = data.error || 'Error'; // Extract the description from the error.

    const description = data.error_description; // Build the error message with the name and description.
  ...
  if (data.errorDetails && Array.isArray(data.errorDetails) && data.errorDetails.length > 0) {
    ...
    message = data.errorDetails.reduce((res, {
      field = '',
      message = ''
    }) => {
      const fieldMsg = `${field}: ${message}`; // Check if `res` is valid, only then add line-breaks to the final message.
...
export function catchError(error, name = 'Error') {
  if (error instanceof Error) {
    ...
    const message = extractErrorMessage(error); // Throw a new Custom Error with the passed in name.
