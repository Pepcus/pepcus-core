...
export const generateMedia = (breakpoints = VALUES) => {
   ...
   * @return {Array}
   */
  const down = key => (...args) => css(["@media (max-width:", "", "){", ";}"], getBreakpointSize(key, breakpoints) - 0.02, UNIT, css(...args));
  /**
  ...
  const up = key => (...args) => css(["@media (min-width:", "", "){", ";}"], getBreakpointSize(key, breakpoints), UNIT, css(...args));
  /**
  ...
  const between = (start, end) => (...args) => css(["@media ( min-width:", "", " and max-width:", "", " ){", "}"], getBreakpointSize(start, breakpoints), UNIT, getBreakpointSize(end, breakpoints) - 0.02, UNIT, css(...args)); // Return the 3 tagged template literal functions
