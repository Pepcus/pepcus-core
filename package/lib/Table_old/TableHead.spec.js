...
describe('<TableHead />', function () {
  var wrapper;
  beforeEach(function () {
    wrapper = shallow(_react.default.createElement(_TableHead.default, {
      schema: schema
    }));
  });
  it('should render a blank table head, without any data present', function () {
    expect(wrapper.find('thead')).to.be.blank();
  });
  it('should render a table head with 1 table row', function () {
    wrapper.setProps({
      data: data
    });
    expect(wrapper.find(_TableRow.default)).to.have.length(1);
  });
  it('should render a table head with 1 table rows and 2 table cells', function () {
    wrapper.setProps({
      data: data
    });
