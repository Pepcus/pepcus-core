...
const bindToAngular = (Component, bindings = [], injectables = [], theme = {}) => {
  ...
  const isValid = isReactComponent(Component); // If no valid React element / component is found, then return.
  ...
  const propTypes = Component.propTypes && Object.keys(Component.propTypes) || []; // Build the Angular `bindings` for the component,
  ...
  class Class extends ngComponent {
    constructor($element, ...injectedProps) {
      super(); // Define the Angular $element.

      this.$element = $element; // Extra props that'll be passed down to the React Component.

      this.injectedProps = {}; // Build a default `theme`.

      this.theme = {}; // Build those extra props.

      injectables.forEach((name, idx) => this.injectedProps[name] = injectedProps[idx]); // Update the default `theme`.
      ...
      if (typeof theme === 'function') {
        ...
        const componentTheme = theme(this.injectedProps); // Only update the `theme` if there is an object present, and it's not empty.

        this.theme = typeof componentTheme === 'object' && !_isEmpty(componentTheme) ? genTheme(componentTheme) : genTheme({});
      } else {
    ...
    render() {
      ReactDOM.render(React.createElement(ThemeProvider, {
        theme: this.theme
      }, React.createElement(Component, Object.assign({}, this.props, this.injectedProps))), this.$element[0]);
    }

  } // Build the Angular Component's controller.


  const controller = [].concat('$element', injectables || [], Class); // Return the Angular JS Component.
