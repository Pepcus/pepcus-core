...
const Button = styled.button.withConfig({
...
})(["-moz-appearance:none;-webkit-appearance:none;background:linear-gradient(to top,#1991eb,#2da1f8);border:", ";color:inherit;cursor:pointer;display:inline-block;outline:none;position:relative;text-align:", ";text-decoration:none;text-transform:initial;transition:box-shadow 0.2s ease-in;user-select:none;vertical-align:middle;&::-moz-focus-inner{border-style:none;}&:disabled{cursor:", ";opacity:", ";}&:focus{outline:none;}", ";", ";", ";"], ({
  border
  ...
  disabledOpacity
}) => disabledOpacity || 0.5, getThemeProps('typography.button'), getThemeProps('Button.styles'), ({
  active,
  borderRadius,
  color: colorProps,
  disabledCursor,
  ...
  let background = getThemeProps(`palette.${colorProps}.gradientColor`, COLORS.PRIMARY_GRADIENT, {
    theme
  });
  ...
  const hoverColor = getThemeProps(`palette.${colorProps}.hoverColor`, color, {
    theme
  });
  ...
  const minWidth = noMinWidth ? 'auto' : getThemeProps('Button.styles.minWidth', '152px', {
    theme
  });
