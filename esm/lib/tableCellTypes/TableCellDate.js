...
function TableCellDate(_ref) {
  let col = _ref.col,
      ...
      rest = _objectWithoutProperties(_ref, ["col", "row", "cellProps"]);

  const format = _get(cellProps, 'format');

  const timezone = _get(cellProps, 'timezone');

  let value = _get(row, cellProps.key) || _get(row, col.id) || _get(cellProps, 'value'); // If the value is a number, we need to fix its length due to Java APIs
  ...
  const date = formatDate(value, format, timezone); // FIX: for the `date-fns` library, where if we send an invalid date, or none at all,
