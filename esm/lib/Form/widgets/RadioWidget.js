...
function RadioWidget(props) {
  ...
  const name = genID('RadioWidget__'); // Extract props from the options.

  const _options$boxProps = options.boxProps,
  ...
  return React.createElement(Box, Object.assign({
    ...
    margin: "0",
    padding: "0 5px"
  }, boxProps), enumOptions.map((option, optionIdx) => {
    const checked = option.value === value;
    const itemDisabled = enumDisabled && enumDisabled.indexOf(option.value) !== -1;
    const key = `${name}__${id}__${optionIdx}`; // Build the single radio.
