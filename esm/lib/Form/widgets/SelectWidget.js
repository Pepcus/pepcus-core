...
const parseAsNumber = value => {
  ...
  const isNumber = typeof num === 'number' && !Number.isNaN(num); // Fantastic, it's a number!
...
const getSelectInputValue = (event, multiple) => {
  if (multiple) {
    return [].slice.call(event.target.options).filter(option => option.selected).map(option => option.value);
  }
...
function SelectWidget(props) {
  ...
  }, !multiple && !schema.default && React.createElement(SelectOption, {
    value: "",
    disabled: true
  }, placeholder), enumOptions.map(({
    value,
    label
  }, idx) => {
