...
import styled, { css } from 'styled-components'; // import Row from 'lib/Row';
...
const TableRowStyled = styled.tr.withConfig({
...
})(["color:inherit;outline:none;vertical-align:middle;", ";", ";"], themeGet('TableRow.styles'), ({
  selected,
  ...
  let selectedCss = '';
  let zebraCss = ''; // Determine the selected row color.

  const selectedColor = themeGet(`palette.${selectedColorProps}.color`, '#FFF')({
    theme
  }); // Determine the zebra-stripe color.

  const zebraColor = themeGet(`palette.${zebraColorProps}.color`, '#FFF')({
    theme
  }); // Update the `selectedCss`

  if (selected) {
    selectedCss = css(["background-color:", ";"], rgba(selectedColor, selectedOpacity));
  } // Update the `zebraCss`


  if (zebra) {
    zebraCss = css(["background-color:", ";"], rgba(zebraColor, zebraOpacity));
  } // Return the concatenated zebra-style with selected-row-style.


  return css(["", ";", ";"], zebraCss, selectedCss);
});
/**
...
const TableRow = React.forwardRef((props, ref) => {
  const selectedProps = props.selected,
        handlers = props.handlers,
        row = props.row,
        rest = _objectWithoutProperties(props, ["selected", "handlers", "row"]); // Determine if the table-row should be selected.


  const selected = typeof selectedProps === 'boolean' ? selectedProps : checkObjectKeys(row, selectedProps, handlers); // Render the table-row

  return React.createElement(TableRowStyled, Object.assign({}, rest, {
    selected: selected,
    ref: ref
  }));
});
