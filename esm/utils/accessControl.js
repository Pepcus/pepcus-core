...
export function accessControl(WrappedComponent) {
  ...
  const AccessControl = React.forwardRef((props, ref) => {
    const acl = props.acl; // Based on the `acl` render the wrapped component.

    return acl ? React.createElement(WrappedComponent, Object.assign({}, props, {
      ref: ref
    })) : null;
  });
  ...
  setDisplayName(wrapDisplayName(WrappedComponent, 'accessControl'))(AccessControl); // Return the composed component.
