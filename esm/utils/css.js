...
export const verifyCSSFlexOrder = value => Boolean(!Number.isNaN(value) && typeof value === 'number');
/**
...
export const verifyCSSColSize = (value, name, theme) => {
  ...
  const columnCount = getThemeProps('grid.columns', 12, {
    theme
...
export const renderTableCellSize = (value, name, theme) => {
  ...
  if (typeof value === 'number' && value <= columnCount) {
    ...
    const width = `${_calcWidth(value, columnCount)}%`; // Determine the styles for the numerical table size.
...
export const renderCSSColSize = (value, name, theme) => {
  ...
  if (typeof value === 'number') {
    // Round to the significant 6 numbers.
    const width = `${_calcWidth(value, columnCount)}%`; // Determine the styles for the numerical column size.
...
export const renderRowGutter = (value, name, theme) => {
  const gutterWidth = _roundGutterWidth(getThemeProps('grid.gutterWidth', 0, {
    theme
  }));
...
export const renderPaletteColor = (cssProperty, paletteKey) => (value, name, theme) => {
  ...
  const color = getThemeProps(`palette.${value}.${paletteKey}`, value, {
    theme
...
export const renderTypographyType = (value, name, theme) => {
  ...
  const type = getThemeProps(`typography.${value}`, null, {
    theme
