...
export function get(obj, path, fallback = null) {
  ...
  const splitPath = typeof path === 'string' ? path.split('.') : Array.isArray(path) ? path : [path]; // Redue to the requested value key.

  const value = splitPath.reduce((acc, key) => acc && acc[key] ? acc[key] : fallback, obj); // Return the value.
