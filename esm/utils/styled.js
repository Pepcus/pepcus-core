...
const _checkValue = (checker = null, ...args) => typeof checker === 'function' ? checker(...args) : true;
/**
...
const _transformValue = (transform = null, value, name, ...args) => value && (typeof transform === 'function' ? transform(value, name, ...args) : `${name}: ${value}`);
/**
...
export const css = (templateStringsList, ...interpolations) => _flatten(styledCSS(templateStringsList, ...interpolations));
/**
...
export const renderStyle = (name = '', value = '', theme = {}, valueCheck = null, valueTransform = null) => {
  ...
  if (value != null && typeof value === 'object') {
    ...
    const breakpointKeys = getThemeProps('breakpoints.keys', {}, {
      theme
    });
    const breakpointValues = getThemeProps('breakpoints.values', {}, {
      theme
    }); // Extract the `media` mixin from the theme.

    const media = getThemeProps('media', {}, {
      theme
    }); // Sort the value keys in the order defined in the breakpoints.

    const sortedKeys = Object.keys(value).sort((a, b) => breakpointKeys.indexOf(a) - breakpointKeys.indexOf(b)); // We'll return a map of css properties.
    ...
    return sortedKeys.reduce((accum, key) => {
      ...
      if (Object.hasOwnProperty.call(breakpointValues, key)) {
        ...
        // CSS is mobile first.
        if (key === 'xs') {
          return _checkValue(valueCheck, value[key], name, theme) ? css`
                              ${_transformValue(valueTransform, value[key], name, theme)};
                          ` : '';
        } // Otherwise, use the `media` mixin for the given breakpoint value.


        const query = _checkValue(valueCheck, value[key], name, theme) ? media.up(key)`
                          ${_transformValue(valueTransform, value[key], name, theme)};
  ...
  return _checkValue(valueCheck, value, name, theme) ? css`
...
export const filterProps = (props = {}) => _omitBy(props, (value, key) => !VALID_ATTRIBUTES_LIST.includes(key));