...
export const down = key => {
  ...
  const endIdx = KEYS.indexOf(key) + 1; // Fetch the upper bound from the values map via the endIdx key.

  const upperBound = VALUES[KEYS[endIdx]]; // If the end index is the same as the keys length, then we have
...
export const between = (start, end) => {
  ...
  const endIdx = KEYS.indexOf(end) + 1; // If the end index is the same as the keys length, then we have
  // reached the end of the keys list and there are no more keys
  ...
  return `@media (min-width: ${VALUES[start]}${UNIT})` + ` and ` + `(max-width: ${VALUES[end] - STEP}${UNIT})`;
};
/**
