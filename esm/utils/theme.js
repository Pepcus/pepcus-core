...
const _getValue = (path, fallback, props) => {
  ...
  const valueFromProps = _get(props, `theme.${path}`, fallback); // Extract the value from the default `THEME`.


  const valueFromTheme = _get(THEME, path, fallback); // Build the final value, checking to see if we have a
  // zero value '0' or empty string '' or any valid value (non null),
  // otherwise falling back to the valueFromTheme.


  let finalValue = valueFromProps || valueFromProps === 0 || valueFromProps === '' ? valueFromProps : valueFromTheme; // If the `finalValue` is an object and is empty,
...
export const getMergedThemeFromSchema = props => {
  ...
  const propsTheme = _get(props, 'theme', {}); // Extract the theme out of the passed in schema.


  const schemaTheme = _get(props, 'schema.theme', {}); // Merge the two themes, overwriting any array values where present.
