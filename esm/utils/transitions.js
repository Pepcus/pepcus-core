...
export const getTransitionProps = (props = {}, options = {}) => {
  ...
  const transitionDelay = _get(style, 'transitionDelay', 0); // Extract the timeout mode if present.


  const modeTimeout = _get(timeout, options.mode); // Build the duration for the transition.


  const duration = transitionDuration || typeof timeout === 'number' ? timeout : modeTimeout; // Build the delay for the transition.
...
export const createNewTransition = (animProps = ['all'], animOptions = {}) => {
  const _animOptions$duration = animOptions.duration,
        durationOption = _animOptions$duration === void 0 ? DURATION.regular : _animOptions$duration,
        _animOptions$easing = animOptions.easing,
        easingOption = _animOptions$easing === void 0 ? EASING.easeInOut : _animOptions$easing,
  ...
  const props = Array.isArray(animProps) ? [...animProps] : [animProps]; // Format the animation duration by rounding it up and adding `ms` string to it.

  const animDuration = typeof durationOption === 'string' ? durationOption : `${Math.round(durationOption)}ms`; // Format the animation delay by rounding it up and adding `ms` string to it.

  const animDelay = typeof delayOption === 'string' ? delayOption : `${Math.round(delayOption)}ms`; // Build the animation based on the given props.

  const animation = props.map(animType => `${animType} ${animDuration} ${easingOption} ${animDelay}`).join(',');
  return animation;
};
/**
...
export const getAutoHeightDuration = height => {
  ...
  const heightConstant = height / 36; // @see {@link https://www.wolframalpha.com/input/?i=(4+%2B+15+*+(x+%2F+36+)+**+0.25+%2B+(x+%2F+36)+%2F+5)+*+10}
