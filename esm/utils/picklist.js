...
export function getPicklistComponent(context = [], props = {}) {
  ...
  const PicklistComponent = !_isEmpty(props) && props.id && context.find(item => getDisplayName(item).includes(props.id)); // Determine whether the found component is a valid React Component.
  ...
  const valid = isValidElement(PicklistComponent) || isClassComponent(PicklistComponent); // If needed, we can assign a `ref` for the wrapped or regular React Component.
