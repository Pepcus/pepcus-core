...
export function getMimeTypeForFile(file) {
  ...
  let mimeType = ''; // Create a new FileReader instance.

  const fileReader = new FileReader(); // Return a Promise.
  ...
  return new Promise((resolve, reject) => {
    ...
    fileReader.onloadend = function (evt) {
      ...
      if (evt.target.readyState === FileReader.DONE) {
        ...
        const uint = new Uint8Array(evt.target.result); // Create a placeholder for all the bytes.

        const bytes = []; // For each of the bytes in the TypedArray,
        ...
        const hex = bytes.join('').toUpperCase(); // Determine the file type based on the generated hex value.
    ...
    const blob = file.slice(0, 4); // Read the first 4 (four) bytes in as an array buffer.
