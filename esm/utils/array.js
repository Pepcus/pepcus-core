...
export function moveArrayItem(array, fromIdx, toIdx, newItem = null) {
  ...
  const itemRemovedArray = [...array.slice(0, fromIdx), ...array.slice(fromIdx + 1, array.length)]; // Add back in the item from the original array from the source index.

  return [...itemRemovedArray.slice(0, toIdx), newItem || array[fromIdx], ...itemRemovedArray.slice(toIdx, itemRemovedArray.length)];
}