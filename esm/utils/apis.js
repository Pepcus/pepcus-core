...
const _successInterceptIdentity = config => config; // const __errorInterceptIdentity = error => Promise.reject(error);
...
async function _errorInterceptIdentity(error) {
  try {
    ...
    const originalRequest = error.config; // Do we have a invalid token error?

    const invalidTokenErrorThrown = error.response && (error.response.status === 401 || error.response.status === 403) && error.response.data.error === 'invalid_token'; // Throw an error if we are re-trying a previous request.
    ...
    if (invalidTokenErrorThrown) {
      ...
      const refreshTokenFunc = _get(error, 'config.refreshToken'); // Only invoke the `refreshTokenFunc` if it's a function and defined.


      if (refreshTokenFunc && typeof refreshTokenFunc === 'function') {
        // Refresh the JWT token.
        const updatedToken = await refreshTokenFunc(error); // Update the headers of the previous API call with the new JWT token.

        originalRequest.headers['Authorization'] = `Bearer ${updatedToken}`; // Redo the last API call.
...
export const extractAPIConfig = (apis, type = 'data') => Array.isArray(apis) ? apis.find(api => Object.prototype.hasOwnProperty.call(api, 'dataType') && api.dataType === type) : apis;
/**
...
export const fetchAPIData = async (config = {}, dataKeysMap = {}) => {
  try {
    ...
    const apiResp = await AXIOS_INSTANCE(config); // Create an empty response object.

    const resp = {}; // If there is no `dataKeysList`, then we'll return the final data
    ...
    if (_isEmpty(dataKeysMap) || typeof dataKeysMap !== 'object') {
      ...
      resp.data = _get(apiResp, 'data', apiResp); // Return the updated `response`.
...
export function buildDataViewJapiApiParams(config = {}) {
  try {
    ...
    const maxPages = rowsPerPage > 0 ? Math.ceil(totalRows / rowsPerPage) : 0; // Determine the offset.

    const offset = maxPages === 0 || maxPages < page ? 0 : (page - 1) * rowsPerPage; // Calculate the sort based on the given order.

    const sort = calculateSort(order, orderBy); // Return the API params.
