...
function accessControlHOC(WrappedComponent) {
  ...
  const AccessControl = React.forwardRef((props, ref) => {
    const acl = props.acl; // Render the component based on the `acl` prop.
  ...
  AccessControl.displayName = wrapDisplayName(WrappedComponent, 'accessControl'); // Return the composed component.
