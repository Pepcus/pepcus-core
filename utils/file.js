...
exports.getMimeTypeForFile = getMimeTypeForFile;
 ...
 */
function _getMimeTypeFromSignature(signature) {
  var nativeMimeType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
...
function getMimeTypeForFile(file) {
  ...
  var mimeType = ''; // Create a new FileReader instance.

  var fileReader = new FileReader(); // Return a Promise.

  return new Promise(function (resolve, reject) {
    // Attach to the `onError` event, and reject the Promsie.
    fileReader.onerror = function () {
      // Abort reading the file.
      fileReader.abort(); // Reject the promise.
    ...
    fileReader.onloadend = function (evt) {
      ...
      if (evt.target.readyState === FileReader.DONE) {
        ...
        var uint = new Uint8Array(evt.target.result); // Create a placeholder for all the bytes.

        var bytes = []; // For each of the bytes in the TypedArray,
        ...
        var hex = bytes.join('').toUpperCase(); // Determine the file type based on the generated hex value.

        mimeType = _getMimeTypeFromSignature(hex, file.type); // Resolve with the buffered mimeType
    ...
    var blob = file.slice(0, 4); // Read the first 4 (four) bytes in as an array buffer.
