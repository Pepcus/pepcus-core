...
class LegislationPublishToField extends Component {
    constructor(props) {
        ...
        completionEvents.forEach((events, idx) => {
            ...
            const keyInFormData = formData.findIndex(completionEvent =>
                completionEvent.completion_event === events.completion_event
            );
        ...
        return completionEvents.map((item, itemIdx) => {
            ...
            const checkboxUpdated =
                Boolean(dataCompletion.needed) === true ? (
                    <input type="checkbox" data-option="updated" data-completion-event={completion_event} checked={dataCompletion.updated} onChange={this.handleSelecting} />
                ) : (
            ...
            return (
                <Row width="auto" alignItems="baseline" key={key} style={{ margin: 0 }}>
                    ...
                    <Col size={4} style={{ justifyContent: 'center', textAlign: 'center'}}>
                        <input type="checkbox" data-option="needed" data-completion-event={completion_event} checked={dataCompletion.needed} onChange={this.handleSelecting} />
                    </Col>
                    <Col size={4} style={{ justifyContent: 'center', textAlign: 'center'}}>
                        {checkboxUpdated}
                    </Col>
    ...
    handleSelecting = evt => {
        ...
        this.setState(prevState => {
            ...
            const keyInFormData = updatedState.formData.findIndex(completionEvent =>
                completionEvent.completion_event === completionEventReal
            );
        ...
        return (
            <Container width="100%">
                ...
                <Row width="auto" alignItems="baseline">
                    <Col size={6}>
                        <div onBlur={this.handleOnBlur} style={{ border: '1px solid #EDEDED', padding: '5px 0'}}>
